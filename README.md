...
- Тема читается блоками: сегмент теории → мини-тест под ним → после
  ...
  скрыты заглушкой с замком). Мини-тест проверяется на месте, без
  перезагрузки; правильные ответы не раскрываются.
- **Интерактивный ноутбук-практика** прямо в портале: тип
  «теория» (читать + кнопка «Прочитал») или «тест» (вводить короткий
  ответ в активные ячейки, остальные read-only). Прохождение
  засчитывается поячеечно и сохраняется. Файл `.ipynb` также можно
  скачать.
- **Линейное прохождение разделов**: пока раздел не пройден целиком,
  следующий показан приглушённо; после завершения появляется большая
  кнопка «Открыть раздел N» — клик разворачивает темы.
...
- Настройка тестов в темах: отметка верных вариантов мини-тестов.
- **Настройка практики (ноутбуков)**: пометить тип (теория/тест) и
  задать эталонные ответы для ячеек (несколько вариантов — по одному
  на строку).
- Реестр пользователей и прогресса; карточка пользователя с разбивкой
  по темам (учитывает и мини-тесты, и ноутбуки).
- **Сброс прохождения** темы — обнуляет и попытки мини-тестов, и
  отметки ноутбука.
...
- Парсинг ноутбуков `.ipynb`: ячейки-ответы определяются тегом nbformat
  `answer`; markdown и code-ячейки рендерятся read-only.
...
| `/course/topic/<id>/practice` | Интерактивный ноутбук-практика |
| `/course/topic/<id>/practice/<n>/check` | Проверка ячейки-ответа |
| `/course/topic/<id>/practice/read` | Отметка «Прочитал» (теория) |
| `/course/notebook/<id>` | Скачивание исходного `.ipynb` |
| `/admin/claim` | Бутстрап первого администратора |
| `/admin/tests`, `/admin/topic/<id>/answers` | Настройка верных ответов мини-тестов |
| `/admin/notebooks`, `/admin/topic/<id>/notebook` | Тип ноутбука и эталоны ответов |
