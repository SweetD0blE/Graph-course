...
- Контент темы целиком из её `.docx`. Тема читается блоками: сегмент
  теории → тест под ним → после сдачи на 100 % открывается следующий
  блок. **Финальный тест** (раздел «Финальный тест» в docx) — последний
  и обязателен для зачёта темы. Тесты проверяются на месте, без
  перезагрузки; правильные ответы не раскрываются.
- **Ознакомительная тема** (в docx нет «Тест»/«Финальный тест») —
  внизу кнопка **«Идти дальше»**, отмечающая тему пройденной.
- **Практика**: тема с `.docx` и подготовленной тетрадкой — кнопка
  **«Скачать файл Jupyter Notebook»**; вопросы практики проверяются на
  сайте (как тест).
- **Замки**: разделы и темы, ещё не открытые, показаны приглушённо с
  иконкой замка и некликабельны; следующая тема/раздел открывается
  после прохождения предыдущих. После прохождения — «К следующей теме».
...
- Настройка ответов в темах: отметка верных вариантов для каждого
  блока — промежуточных «Тест» и «Финальный тест» (на одной странице).
- Реестр пользователей и прогресса; карточка пользователя с разбивкой
  по темам.
- **Сброс прохождения** темы (обнуляет попытки пользователя по теме).
...
- Импорт сотрудников из `SVA_persons.xlsx` и плана курса из
  ...
  (`mammoth`) с разбиением на блоки по разметке «Тест»/«Финальный тест».
  Переимпорт по изменению docx (хеш) с переносом отметок ответов.
- `.ipynb` из плана курса — скачиваемый компаньон темы-практики.
...
| `/course/` | Список разделов и тем (с замками) |
| `/course/topic/<id>` | Тема: блоки теории + тесты, прогрессивное открытие |
| `/course/topic/<id>/block/<n>/test` | Проверка теста блока (вкл. финальный) |
| `/course/topic/<id>/complete` | Отметка «Идти дальше» (ознакомительная тема) |
| `/course/notebook/<id>` | Скачивание `.ipynb` темы-практики |
| `/admin/claim` | Бутстрап первого администратора |
| `/admin/tests`, `/admin/topic/<id>/answers` | Настройка верных ответов (тесты и финал) |
| `/admin/users`, `/admin/users/<id>` | Реестр и карточка пользователя |
| `/admin/users/<id>/reset/<topic_id>` | Сброс прохождения темы |
...
app/progress.py         is_gradable, topic_test_blocks, passed_blocks,
                        ...
                        section_completed, course_topic_states
